...
export const getReg = (keyword: string) => (operator = regAND) => buildRegExp(keyword, operator);

export const filterMatchingPosts = (posts: Post[]) => (keyword: string): Post[] => {
    ...
    const matchWith = (regExp: RegExp | null) => regExp ? posts.filter((p) => regExp.test(p.title)) : [];
