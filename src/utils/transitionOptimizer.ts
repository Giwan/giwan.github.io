...
import {detectDeviceCapabilities} from './transition/deviceDetector';
import {
 ...
 */
export function calculateRecommendations(capabilities: DeviceCapabilities, metrics: {
    frameRate: number;
    memoryUsage?: number;
}, preferences: {
    respectReducedMotion: boolean;
}, isLowBattery: boolean, isSlowNetwork: boolean): OptimizationRecommendations {
    ...
    if (preferences.respectReducedMotion && globalThis.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        shouldSimplifyAnimations = true;
...
export function getOptimizationRecommendations(): OptimizationRecommendations {
    return calculateRecommendations(stateDeviceCapabilities, performanceMonitor.getCurrentMetrics(), transitionPreferences.getPreferences(), isBatteryLow(), isSlowNetwork());
}

/**
 * Apply optimizations to the DOM
 */
export function applyOptimizationsToDOM(recommendations: OptimizationRecommendations, settings: OptimizationSettings, capabilities: DeviceCapabilities): void {
    ...
    root.setAttribute('data-current-fps', Math
        .round(currentMetrics.frameRate)
        .toString());
    
    if (!isUndefined(currentMetrics.memoryUsage))
        root.setAttribute('data-memory-usage', Math
            .round(currentMetrics.memoryUsage * 100)
            .toString());
}
...
export function startOptimization(): void {
    if (typeof window === 'undefined' || stateIsOptimizing)
        return;
    ...
    stateOptimizationInterval = globalThis.setInterval(optimizeTransitions, 5000);
}
