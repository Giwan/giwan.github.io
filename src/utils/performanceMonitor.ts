...
export class PerformanceMonitor {
    ...
    private readonly FRAME_RATE_THRESHOLD = 30;
    // FPS below which we consider performance poor
    private readonly DROPPED_FRAME_THRESHOLD = 5;
    // Number of dropped frames that triggers fallback
        ...
        const measureFrameRate = (currentTime: number) => {
            if (this.isMonitoring) {
                ...
                if (this.currentTransitionData) {
                    ...
                    if (deltaTime > 16.67)
                        this.currentTransitionData.droppedFrames = (this.currentTransitionData.droppedFrames || 0) + 1;
                    
                    // Track worst frame time
                    if (!this.currentTransitionData.worstFrameTime || deltaTime > this.currentTransitionData.worstFrameTime)
                ...
                if (!(frameCount % 30) && this.performanceChangeCallbacks.length > 0) {
    ...
    public getCurrentMetrics(): PerformanceMetrics {
        ...
        const averageFrameRate = recentFrameRates.length > 0 ? recentFrameRates.reduce((sum, rate) => sum + rate, 0) / recentFrameRates.length : 60;
        ...
        return {
            ...
            droppedFrames,
            transitionDuration: this.currentTransitionData ? performance.now() - (this.currentTransitionData.startTime ?? 0) : 0,
    ...
    private detectLowPowerMode(): boolean {
        ...
        const indicators = [
            ...
            navigatorWithExtensions.deviceMemory && navigatorWithExtensions.deviceMemory <= 2,
    ...
    private getAverageFrameRate(): number {
        if (!this.frameRateHistory.length)
            return 60;
    ...
    private checkPerformanceAndTriggerFallback(data: TransitionPerformanceData): void {
        const shouldTriggerFallback = data.averageFrameRate < this.FRAME_RATE_THRESHOLD || data.droppedFrames > this.DROPPED_FRAME_THRESHOLD || this.MEMORY_THRESHOLD < (this.getMemoryUsage() || 0);
    ...
    public getRecommendedIntensity(data?: TransitionPerformanceData | PerformanceMetrics): TransitionIntensity {
        ...
        if (deviceCapabilities.isLowPowerMode || deviceCapabilities.hardwareConcurrency <= 2 || (deviceCapabilities.deviceMemory && deviceCapabilities.deviceMemory <= 2))
        ...
        if (frameRate >= 55 && deviceCapabilities.hardwareConcurrency >= 8 && (!deviceCapabilities.deviceMemory || deviceCapabilities.deviceMemory >= 8))
     ...
     */
    private handleNavigationTiming(): void {
        // Store navigation performance data
        if (this.currentTransitionData?.deviceInfo) {
    ...
    /**
      * Handle paint timing
      */
    private handlePaintTiming(_entry: PerformancePaintTiming): void {
        // Track paint events during transitions
        if (this.isMonitoring && _entry.name === 'first-contentful-paint') {
    ...
    private notifyPerformanceChange(metrics: PerformanceMetrics): void {
        for (const callback of this.performanceChangeCallbacks) {
            callback(metrics);
        }
    }
    /**
