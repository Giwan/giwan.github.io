...
// Augment Navigator to include getBattery method
export class MobileTransitionOptimizer {
   ...
   */
    private detectDeviceCapabilities(): DeviceCapabilities {
        ...
        const orientation = (typeof window !== 'undefined') ? (globalThis.innerHeight > globalThis.innerWidth ? 'portrait' : 'landscape') : 'portrait';

        const hardwareConcurrency = (typeof navigator !== 'undefined') ? (navigator.hardwareConcurrency || 4) : 4;

        // @ts-ignore
        const deviceMemory = (typeof navigator !== 'undefined' && 'deviceMemory' in navigator) ? (navigator.deviceMemory as number | undefined) : undefined;

        // Get connection info without using any type
    ...
    private detectNetworkCondition(): NetworkCondition {
        ...
        if (typeof navigator !== 'undefined') {
            ...
            if (nav.connection)
                ...
                connection = nav.mozConnection;
            else if (nav.webkitConnection)
                connection = nav.webkitConnection;
        }
   ...
   */
    private setupOrientationChangeHandling(): void {
        const handleOrientationChange = () => {
            const newOrientation = (typeof window !== 'undefined') ? (globalThis.innerHeight > globalThis.innerWidth ? 'portrait' : 'landscape') : 'portrait';
            ...
            if (newOrientation !== this.deviceCapabilities.orientation) {
                ...
                for (const listener of this.orientationChangeListeners)
                    listener();
            }
        };
        ...
        const handleNetworkChange = () => {
            ...
            if (newCondition.effectiveType !== this.networkCondition.effectiveType) {
                ...
                for (const listener of this.networkChangeListeners)
                    listener(newCondition);
            }
        };

        if (connection && typeof (connection as EventTarget).addEventListener === 'function')
            (connection as EventTarget).addEventListener('change', handleNetworkChange);
    }
    /**
   * Set up battery monitoring
   */
    private setupBatteryMonitoring(): void {
        if (typeof navigator !== 'undefined' && ('getBattery' in navigator))
            navigator
                .getBattery()
                .then((battery: { charging: boolean; level: number; addEventListener: (event: string, listener: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions) => void; removeEventListener: (event: string, listener: EventListenerOrEventListenerObject, options?: boolean | EventListenerOptions) => void }) => {
                    const updateBatteryStatus = () => {
                        const isLowBattery = battery.level < 0.2 || !battery.charging && battery.level < 0.3;
                        ...
                        if (isLowBattery !== this.deviceCapabilities.isLowBattery) {
                            ...
                            for (const listener of this.batteryChangeListeners)
                                listener(isLowBattery);
                        }
                    };
                    ...
                    if (battery && isFn(battery.addEventListener)) {
   ...
   */
    private isOrientationChanging(): boolean {
        ...
        const aspectRatio = (typeof window !== 'undefined') ? globalThis.innerWidth / globalThis.innerHeight : 1;
   ...
   */
    public getPWATransitionSettings(): {
        duration: number;
        easing: string;
    } {
        if (!this.deviceCapabilities.isPWA)
