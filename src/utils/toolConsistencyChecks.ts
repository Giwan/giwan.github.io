 ...
 */
export function validateCategoryConsistency(tools: TTool[], fileName?: string): ConsistencyResult {
    ...
    for (const [i, tool] of tools.entries()) {
        ...
        if (!validCategories.includes(tool.category)) {
            ...
            result.errors.push(`${toolPrefix}: Invalid category '${tool.category}'. Valid categories: ${validCategories.join(', ')}`);
            result.isConsistent = false;
        }
    }
    ...
    for (const invalidCategory of invalidCategories) {
        ...
        if (suggestions.length > 0)
            result.suggestions.push(`Invalid category '${invalidCategory}' might be: ${suggestions.join(', ')}`);
    }
 ...
 */
export function validateLabelConsistency(tools: TTool[], fileName?: string): ConsistencyResult {
    const result: ConsistencyResult = {
        isConsistent: true,
        errors: [],
        warnings: [],
    ...
    for (const [i, tool] of tools.entries()) {
        ...
        for (const label of tool.labels) {
            ...
            if (!validLabels.includes(label)) {
                ...
                result.warnings.push(`${toolPrefix}: Label '${label}' is not in predefined labels list`);
            }
        }
    ...
    for (const invalidLabel of invalidLabels) {
        ...
        if (suggestions.length > 0)
            result.suggestions.push(`Invalid label '${invalidLabel}' might be: ${suggestions.join(', ')}`);
    }

    // Report rarely used labels
    const rareLabels = Array
        .from(labelUsageCount.entries())
        .filter(([, count]) => count === 1)
        .map(([label]) => label);

    if (rareLabels.length > 0)
        result.suggestions.push(`Labels used only once (consider if they're necessary): ${rareLabels.join(', ')}`);
 ...
 */
export function detectDuplicateTools(toolsData: {
    tools: TTool[];
    fileName?: string;
}[]): DuplicateResult {
 ...
 */
export function validateAllToolConsistency(toolsData: {
    tools: TTool[];
    fileName?: string;
}[]): {
    ...
    const hasErrors = !categoryConsistency.isConsistent || !labelConsistency.isConsistent || duplicates.hasDuplicates;

    const hasWarnings = categoryConsistency.warnings.length > 0 || labelConsistency.warnings.length > 0;
...
function findSimilarCategories(target: string, validCategories: string[]): string[] {
    ...
    return validCategories
        .filter((category) => {
            ...
            return categoryLower.includes(targetLower) || targetLower.includes(categoryLower) || levenshteinDistance(targetLower, categoryLower) <= 2;
        })
...
function findSimilarLabels(target: string, validLabels: string[]): string[] {
    ...
    return validLabels
        .filter((label) => {
            ...
            return labelLower.includes(targetLower) || targetLower.includes(labelLower) || levenshteinDistance(targetLower, labelLower) <= 2;
        })
        .slice(0, 3); // Limit to top 3 suggestions
}

function findExactDuplicates(allTools: {
    tool: TTool;
    index: number;
    fileName?: string;
}[]): {
...
function findTitleDuplicates(allTools: {
    tool: TTool;
    index: number;
    fileName?: string;
}[]): {
...
function findUrlDuplicates(allTools: {
    tool: TTool;
    index: number;
    fileName?: string;
}[]): {
