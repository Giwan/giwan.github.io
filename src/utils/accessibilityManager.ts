...
export class AccessibilityManager {
    ...
    private setupTransitionEventListeners(): void {
        ...
        document.addEventListener(
            "astro:after-swap",
            this.handleTransitionComplete.bind(this),
        );
        document.addEventListener(
            "astro:page-load",
            this.handlePageLoad.bind(this),
        );
    }
    ...
    private handleTransitionStart(event: Event): void {
        ...
        if (this.preferences.screenReaderAnnouncements) {
            ...
            const fromPath =
                customEvent.detail?.from?.pathname || "current page";
                ...
                if (href && href.startsWith("#")) {
                    ...
                    if (target) {
                        ...
                        (target as HTMLElement).scrollIntoView({
                            behavior: this.preferences.reducedMotion
                                ? "auto"
                                : "smooth",
                            block: "start",
                        });
                        ...
                        if (!originalTabIndex) {
                            setTimeout(() => {
                                (target as HTMLElement).removeAttribute(
                                    "tabindex",
                                );
                            }, 100);
                        }
    ...
    private handleEscapeKey(): void {
        ...
        const openModals = document.querySelectorAll(
            '[role="dialog"][aria-hidden="false"], .modal.open, .panel.open',
        );
    ...
    private handleFocusDuringTransition(event: FocusEvent): void {
        ...
        if (target && this.preferences.screenReaderAnnouncements) {
            ...
            if (
                elementLabel &&
                (elementType === "button" || elementType === "a" || elementRole)
            ) {
                // Don't announce every focus change, only important ones
                if (
                    target.matches(
                        '[aria-describedby], [aria-expanded], [role="button"], [role="link"]',
                    )
                ) {
                    this.announce({
