...
export class AccessibilityManager {
    ...
    private loadPreferences(): AccessibilityPreferences {
        const stored = typeof localStorage !== 'undefined' && typeof localStorage.getItem === 'function' ? localStorage.getItem('accessibility-preferences') : null;
    ...
    private isElementVisible(element: HTMLElement): boolean {
        ...
        return (rect.width > 0 && rect.height > 0 && globalThis.getComputedStyle(element).visibility !== 'hidden');
    }
    /**
    ...
    private updateLandmarkTransitionNames(): void {
        ...
        for (const [index, link] of skipLinks.entries()) {
            if (!link.style.viewTransitionName)
                link.style.viewTransitionName = `skip-link-${index}`;
        }
    }
    /**
    ...
    public announce(announcement: NavigationAnnouncement): void {
        if (!this.announcer || !this.preferences.screenReaderAnnouncements)
            return;
    ...
    private handleTabDuringTransition(event: KeyboardEvent): void {
        ...
        const focusableElements = mainContent.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');
    ...
    private handleFocusDuringTransition(event: FocusEvent): void {
        ...
        if (target && this.preferences.screenReaderAnnouncements) {
            ...
            if (elementLabel && (elementType === 'button' || elementType === 'a' || elementRole) && target.matches('[aria-describedby], [aria-expanded], [role="button"], [role="link"]'))
                this.announce({
