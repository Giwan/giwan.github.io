...
export enum PageRelationship {
    SIBLING = 'sibling',
    // Same level (blog post to blog post)
...
export class TransitionController {
   ...
   */
    public detectNavigationContext(fromPath: string, toPath: string): NavigationContext {
        ...
        const relationship = this.analyzePageRelationship(fromPath, toPath);
   ...
   */
    private isBackwardNavigation(fromPath: string, toPath: string): boolean {
        ...
        return backwardPatterns.some((pattern) => pattern.from.test(fromPath) && pattern.to.test(toPath));
    }
    /**
   ...
   */
    private analyzePageRelationship(fromType: PageType, toType: PageType): PageRelationship {
        ...
        if (contextualPairs.some(([type1, type2]) => (fromType === type1 && toType === type2) || (fromType === type2 && toType === type1)))
   ...
   */
    private getUserAgentInfo(): UserAgentInfo {
        ...
        const navWithExtensions = navigator as NavigatorWithExtensions;
        const connection = navWithExtensions.connection || navWithExtensions.mozConnection || navWithExtensions.webkitConnection;
   ...
   */
    private applyMobileOptimizations(): void {
        ...
        if (!isUndefined(deviceCapabilities.batteryLevel))
            root.setAttribute('data-battery-level', Math
                .round(deviceCapabilities.batteryLevel * 100)
                .toString());
    ...
    */
    public applyTransitionContext(context: NavigationContext): void {
        const root = document.documentElement;
   ...
   */
    private updateMetrics(context: NavigationContext): void {
        ...
        this.metrics.averageDuration = (this.metrics.averageDuration * (this.metrics.totalTransitions - 1) + estimatedDuration) / this.metrics.totalTransitions;
    }
    /**
   ...
   */
    public getCurrentContext(): NavigationContext | null {
        ...
        const currentPath = this.navigationHistory.at(-1);
        const previousPath = this.navigationHistory.at(-2);
   ...
   */
    private applyPerformanceOptimizations(): void {
        ...
        if (!isUndefined(currentMetrics.memoryUsage)) {
            const memoryLevel = currentMetrics.memoryUsage > 0.8 ? 'high' : currentMetrics.memoryUsage > 0.6 ? 'medium' : 'low';
        ...
        if ('getBattery' in navigator) {
            const navWithBattery = navigator as { getBattery(): Promise<{ level: number; charging: boolean }> };
            navWithBattery
                .getBattery()
                .then((battery) => {
                    root.setAttribute('data-battery-level', Math
   ...
   */
    private updatePerformanceMetrics(performanceData: TransitionPerformanceData): void {
        ...
        this.metrics.averageDuration = (this.metrics.averageDuration * (this.metrics.totalTransitions - 1) +
        (performanceData.endTime - performanceData.startTime)) / this.metrics.totalTransitions;

        // Calculate failure rate based on performance thresholds
        const isFailure = performanceData.averageFrameRate < 30 || performanceData.droppedFrames > 5;

        if (isFailure) {
            this.metrics.failureRate = (this.metrics.failureRate * (this.metrics.totalTransitions - 1) + 1) / this.metrics.totalTransitions;
            return;
        }

        this.metrics.failureRate = (this.metrics.failureRate * (this.metrics.totalTransitions - 1)) / this.metrics.totalTransitions;
    }
    /**
   ...
   */
    private triggerSoundEffect(): void {
        ...
        try {
            const AudioContextClass = globalThis.AudioContext || (window as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
   ...
   */
    public getEnhancedMetrics(): TransitionMetrics & {
