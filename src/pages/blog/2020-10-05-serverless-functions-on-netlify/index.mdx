---
description: "Setup a simple demo login function to see serverless functions in action"
title: "Getting started with serverless functions on Netlify"

createdDate: "2020-10-06"
...
For now the login button will call the end-point with the credentials. If HTTP 200 is received the front-end redirects to the /admin page.

```bash
npm i react-router react-router-dom
```

Update `index.js` so there is support for react-router from the root. Wrap `<App />` in `<BrowserRouter>`.
