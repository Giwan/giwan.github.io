---
description: "What if a user's passwords were a list of UUIDs instead? A list that can be generated in a less traditional means."
...
The end result is that the actual password leaving the client device and being sent over the network is an intermediate list. When using SSL to secure the connection this might seem redundant. It serves a higher purpose however.
...
### Passwords from one compromised site, pose less of a risk on other sites.

If a user uses the password "hello" on site A. That password is sent over the wire as a list of UUIDs. Ultimately it's stored as a different string.
On site B, the user uses the same password. If site B is compromised the password that is obtained can't be easily uses to compare with Site A.
