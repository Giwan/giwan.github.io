---
description: "Using for loops is basic however JS offers several ways of looping with functions."
...
Another way of doing this is by using the built in `every` function. It loops over the give array and takes a test function as an argument. If every item of the array passes the test, it returns true.
...
It effectively does the same thing except it uses `every` instead of a for loop. Note `return isUnique`. If `false` is returned then the loop stops.
