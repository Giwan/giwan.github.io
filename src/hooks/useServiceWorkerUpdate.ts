...
import {handlePWAError, PWAError} from '../utils/pwa';
...
export const useServiceWorkerUpdate = (): ServiceWorkerUpdateState => {
    ...
    const setupServiceWorkerHandling = async () => {
        try {
            ...
            registration.addEventListener('updatefound', () => {
                ...
                if (newWorker) {
                    ...
                    newWorker.addEventListener('error', () => {
                        handleError(Error('Service worker installation failed'), 'installation');
                    });
                }
            });
            ...
            registration.addEventListener('error', () => {
                handleError(Error('Service worker runtime error'), 'runtime');
            });
        } catch(error) {
    ...
    useEffect(() => {
        ...
        globalThis.addEventListener('pwa-error', handlePWAError as EventListener);

        return () => {
            navigator.serviceWorker?.removeEventListener('message', handleMessage);
            globalThis.removeEventListener('pwa-error', handlePWAError as EventListener);
        };
    }, []);
    ...
    const dismissNotification = () => {
        try {
            ...
            if (waitingWorker)
                localStorage.setItem('sw-update-dismissed', Date
                    .now()
                    .toString());
        } catch(error) {
