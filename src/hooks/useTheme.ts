...
if (!isServer())
    import('../utils/transitionController').then(noop);
    ...
    useEffect(() => {
        const updateTheme = () => {
            ...
            if (theme === THEME_VALUES.SYSTEM)
                effectiveTheme = globalThis.matchMedia(MEDIA_QUERY).matches ? THEME_VALUES.DARK : THEME_VALUES.LIGHT;
            else
            ...
            const isPageTransitioning = document.documentElement.hasAttribute(DATA_TRANSITION_ATTR);
             ...
             if (typeof document !== 'undefined' && 'startViewTransition' in document && !isPageTransitioning) {
                 try {
                     (document as Document & { startViewTransition: (callback: () => void) => void }).startViewTransition(() => updateDOMWithTransition());
                 } catch {
