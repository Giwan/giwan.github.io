...
export async function loadMoreArticles(retryAttempt = 0): Promise<void> {
    ...
    try {
        ...
        if (globalThis.__ARTICLE_DATA__)
            globalThis.__ARTICLE_DATA__.loadedCount = (globalThis.__ARTICLE_DATA__.loadedCount || 0) + newArticles.length;
    } catch(error) {
        ...
        if (retryAttempt < MAX_RETRY_ATTEMPTS) {
            ...
            await new Promise((resolve) => setTimeout(resolve, 1000 * 2 ** retryAttempt));
            return await loadMoreArticles(retryAttempt + 1);
        }
 ...
 */
export async function retryLoadingArticles(): void {
...
async function fetchArticles(page: number, limit: number): Promise<Article[]> {
    ...
    const {allArticles, totalArticles} = globalThis.__ARTICLE_DATA__ || {
        allArticles: [],
        totalArticles: 0,
    };
